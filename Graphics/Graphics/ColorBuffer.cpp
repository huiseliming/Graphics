...
void ColorBuffer::CreateDerivedViews(ID3D12Device* Device, DXGI_FORMAT Format, uint32_t ArraySize, uint32_t NumMips)
{
    ...
    {
        SAFE_DISCARD_RTV_DESCRIPTOR(m_RTVHandle);
        m_RTVHandle = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        m_SRVHandle = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    }
    ...
    for (uint32_t i = 0; i < NumMips; ++i)
    {
        if (m_UAVHandle[i].ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
            m_UAVHandle[i] = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
{
	SAFE_DISCARD_SRV_DESCRIPTOR(m_SRVHandle);
	SAFE_DISCARD_RTV_DESCRIPTOR(m_RTVHandle);
	for (size_t i = 0; i < 12; i++)
		SAFE_DISCARD_UAV_DESCRIPTOR(m_UAVHandle[i]);
}

void ColorBuffer::CreateFromSwapChain( const std::wstring& Name, ID3D12Resource* BaseResource )
{
    AssociateWithResource(GlobalVariable<ID3D12Device>::Get(), Name, BaseResource, D3D12_RESOURCE_STATE_PRESENT);
    ...
    SAFE_DISCARD_RTV_DESCRIPTOR(m_RTVHandle);
    m_RTVHandle = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
    GlobalVariable<ID3D12Device>::Get()->CreateRenderTargetView(m_pResource.Get(), nullptr, m_RTVHandle);
}
    ...
    DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem)
{
    ...
    CreateTextureResource(GlobalVariable<ID3D12Device>::Get(), Name, ResourceDesc, ClearValue, VidMem);
    CreateDerivedViews(GlobalVariable<ID3D12Device>::Get(), Format, 1, NumMips);
}
    ...
    DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem )
{
    ...
    CreateTextureResource(GlobalVariable<ID3D12Device>::Get(), Name, ResourceDesc, ClearValue, VidMem);
    CreateDerivedViews(GlobalVariable<ID3D12Device>::Get(), Format, ArrayCount, 1);
}
