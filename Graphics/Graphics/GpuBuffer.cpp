...
GpuBuffer::~GpuBuffer()
{
	SAFE_DISCARD_UAV_DESCRIPTOR(m_UAV);
	SAFE_DISCARD_SRV_DESCRIPTOR(m_SRV);
	Destroy();
}
...
void GpuBuffer::Create( const std::wstring& name, uint32_t NumElements, uint32_t ElementSize, const void* initialData )
{
    ...
    THROW_HR_EXCEPT_IF_FAILED( 
        GlobalVariable<ID3D12Device>::Get()->CreateCommittedResource( &HeapProps, D3D12_HEAP_FLAG_NONE,
    ...
    const void* initialData)
{
    ...
    THROW_HR_EXCEPT_IF_FAILED(GlobalVariable<ID3D12Device>::Get()->CreatePlacedResource(pBackingHeap, HeapOffset, &ResourceDesc, m_UsageState, nullptr, IID_PPV_ARGS(&m_pResource)));
...
D3D12_CPU_DESCRIPTOR_HANDLE GpuBuffer::CreateConstantBufferView(uint32_t Offset, uint32_t Size) const
{
    ...
    //GlobalVariable<ID3D12Device>::Get()->CreateConstantBufferView(&CBVDesc, hCBV);
    //return hCBV;
...
void ByteAddressBuffer::CreateDerivedViews(void)
{
    ...
    if (m_SRV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
        m_SRV = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    GlobalVariable<ID3D12Device>::Get()->CreateShaderResourceView(m_pResource.Get(), &SRVDesc, m_SRV);
    ...
    if (m_UAV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
        m_UAV = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    GlobalVariable<ID3D12Device>::Get()->CreateUnorderedAccessView( m_pResource.Get(), nullptr, &UAVDesc, m_UAV );
}
...
void StructuredBuffer::CreateDerivedViews(void)
{
    ...
    if (m_UAV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
        m_UAV = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    GlobalVariable<ID3D12Device>::Get()->CreateUnorderedAccessView(m_pResource.Get(), m_CounterBuffer.GetResource(), &UAVDesc, m_UAV);
}
