...
public:
    ...
    void DiscardDescriptorHeaps(D3D12_DESCRIPTOR_HEAP_TYPE HeapType, uint64_t FenceValueForReset, const std::vector<ID3D12DescriptorHeap*>& UsedHeaps);
    ...
    const uint32_t kNumDescriptorsPerHeap = 1024;
private:
    std::mutex sm_Mutex[2];
    std::vector<Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>> sm_DescriptorHeapPool[2];
    std::queue<std::pair<uint64_t, ID3D12DescriptorHeap*>> sm_RetiredDescriptorHeaps[2];
    std::queue<ID3D12DescriptorHeap*> sm_AvailableDescriptorHeaps[2];
};
...
public:
    ...
    {
        sm_DynamicDescriptorHeapManager.DestroyAll();
    }
...
private:
    ...
    // Static members
    static DynamicDescriptorHeapManager sm_DynamicDescriptorHeapManager;
    ...
    {
        return (m_CurrentHeapPtr != nullptr && m_CurrentOffset + Count <= sm_DynamicDescriptorHeapManager.kNumDescriptorsPerHeap);
    }
