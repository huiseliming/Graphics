...
{
	SAFE_DISCARD_SRV_DESCRIPTOR(m_hDepthSRV);
	SAFE_DISCARD_SRV_DESCRIPTOR(m_hStencilSRV);
	for (size_t i = 0; i < 4; i++)
		SAFE_DISCARD_DSV_DESCRIPTOR(m_hDSV[i]);
}
...
void DepthBuffer::Create( const std::wstring& Name, uint32_t Width, uint32_t Height, DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr )
{
    ...
    CreateTextureResource(GlobalVariable<ID3D12Device>::Get(), Name, ResourceDesc, ClearValue, VidMemPtr);
    CreateDerivedViews(GlobalVariable<ID3D12Device>::Get(), Format);
}
...
void DepthBuffer::CreateDerivedViews( ID3D12Device* Device, DXGI_FORMAT Format)
{
    ...
    {
        m_hDSV[0] = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
        m_hDSV[1] = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
    }
    ...
    if (stencilReadFormat != DXGI_FORMAT_UNKNOWN)
    {
        ...
        {
            m_hDSV[2] = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
            m_hDSV[3] = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
        }
    ...
    if (m_hDepthSRV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
        m_hDepthSRV = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    ...
    if (stencilReadFormat != DXGI_FORMAT_UNKNOWN)
    {
        if (m_hStencilSRV.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
            m_hStencilSRV = ALLOCATE_DESCRIPTOR(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
