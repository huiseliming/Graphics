...
void RootSignature::Finalize(const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS Flags)
{
    ...
    if (firstCompile)
    {
        ...
        THROW_BLOB_EXCEPT( D3D12SerializeRootSignature(&RootDesc, D3D_ROOT_SIGNATURE_VERSION_1,
            pOutBlob.GetAddressOf(), pErrorBlob.GetAddressOf()), pErrorBlob);

        THROW_HR_EXCEPT_IF_FAILED(GlobalVariable<ID3D12Device>::Get()->CreateRootSignature(1, pOutBlob->GetBufferPointer(), pOutBlob->GetBufferSize(),
