...
void GraphicsPSO::Finalize()
{
    ...
    if (firstCompile)
    {
        THROW_HR_EXCEPT_IF_FAILED(GlobalVariable<ID3D12Device>::Get()->CreateGraphicsPipelineState(&m_PSODesc, IID_PPV_ARGS(&m_PSO)) );
...
void ComputePSO::Finalize()
{
    ...
    if (firstCompile)
    {
        THROW_HR_EXCEPT_IF_FAILED(GlobalVariable<ID3D12Device>::Get()->CreateComputePipelineState(&m_PSODesc, IID_PPV_ARGS(&m_PSO)) );
