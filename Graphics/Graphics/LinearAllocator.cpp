...
LinearAllocationPage* LinearAllocatorPageManager::RequestPage()
{
    ...
    while (!m_RetiredPages.empty() && GlobalVariable<CommandQueueManager>::Get()->IsFenceComplete(m_RetiredPages.front().first))
    {
...
void LinearAllocatorPageManager::FreeLargePages( uint64_t FenceValue, const vector<LinearAllocationPage*>& LargePages )
{
    ...
    while (!m_DeletionQueue.empty() && GlobalVariable<CommandQueueManager>::Get()->IsFenceComplete(m_DeletionQueue.front().first))
    {
...
LinearAllocationPage* LinearAllocatorPageManager::CreateNewPage( size_t PageSize  )
{
    ...
    THROW_HR_EXCEPT_IF_FAILED(GlobalVariable<ID3D12Device>::Get()->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_NONE,
