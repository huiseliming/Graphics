...
DynamicDescriptorHeapManager DynamicDescriptorHeap::sm_DynamicDescriptorHeapManager;

ID3D12DescriptorHeap* DynamicDescriptorHeapManager::RequestDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
{
    uint32_t idx = HeapType == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER ? 1 : 0;
    std::lock_guard<std::mutex> LockGuard(sm_Mutex[idx]);

    while (!sm_RetiredDescriptorHeaps[idx].empty() && GlobalVariable<CommandQueueManager>::Get()->IsFenceComplete(sm_RetiredDescriptorHeaps[idx].front().first))
    {
    ...
    else
    {
        ...
        THROW_HR_EXCEPT_IF_FAILED(GlobalVariable<ID3D12Device>::Get()->CreateDescriptorHeap(&HeapDesc, IID_PPV_ARGS(&HeapPtr)));
        sm_DescriptorHeapPool[idx].emplace_back(HeapPtr);
        return HeapPtr.Get();
    }
}

void DynamicDescriptorHeapManager::DiscardDescriptorHeaps( D3D12_DESCRIPTOR_HEAP_TYPE HeapType, uint64_t FenceValue, const std::vector<ID3D12DescriptorHeap*>& UsedHeaps )
{
    uint32_t idx = HeapType == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER ? 1 : 0;
    std::lock_guard<std::mutex> LockGuard(sm_Mutex[idx]);
...
{
    sm_DynamicDescriptorHeapManager.DiscardDescriptorHeaps(m_DescriptorType, fenceValue, m_RetiredHeaps);
    ...
    : m_OwningContext(OwningContext), m_DescriptorType(HeapType)
{
    m_CurrentHeapPtr = nullptr;
    m_CurrentOffset = 0;
    m_DescriptorSize = GlobalVariable<ID3D12Device>::Get()->GetDescriptorHandleIncrementSize(HeapType);
}
...
inline ID3D12DescriptorHeap* DynamicDescriptorHeap::GetHeapPointer()
{
    if (m_CurrentHeapPtr == nullptr)
    {
        ...
        m_CurrentHeapPtr = sm_DynamicDescriptorHeapManager.RequestDescriptorHeap(m_DescriptorType);
    ...
    void (STDMETHODCALLTYPE ID3D12GraphicsCommandList::*SetFunc)(UINT, D3D12_GPU_DESCRIPTOR_HANDLE))
{
    ...
    for (uint32_t i = 0; i < StaleParamCount; ++i)
    {
        ...
        while (_BitScanForward64(&SkipCount, SetHandles))
        {
            ...
            {
                GlobalVariable<ID3D12Device>::Get()->CopyDescriptors(
...
D3D12_GPU_DESCRIPTOR_HANDLE DynamicDescriptorHeap::UploadDirect( D3D12_CPU_DESCRIPTOR_HANDLE Handle )
{
    ...
    GlobalVariable<ID3D12Device>::Get()->CopyDescriptorsSimple(1, DestHandle.GetCpuHandle(), Handle, m_DescriptorType);
